...
export default function EditarProductoView() {
  ...
  const manejarSubmit = async (e) => {
    e.preventDefault()
    if(typeof imagen !== undefined){ // si es diferente de undefined entonces hay imagen (cuando se carga una imagen)
      const urlArchivo = await subirArchivo(imagen)
      await editarProducto({...value, prod_imagen:urlArchivo}, id)
    }else{
      await editarProducto(value, id) // si es que no tiene imagen, me limito a actualizar lo demas
    }    
    await Swal.fire({
      icon:'success',
  ...
  return (
    <div>
      <h1 className="mt-3 mb-3 text-success text-center">Editar Producto</h1>
      <FormProducto
        value={value}
